...
function parseResumeId(req) {
  ...
  const match = path.match(/\/resumes\/(\d+)\/duplicate(?:\/)?$/);
...
export default withApi(async (req, res) => {
  ...
  try {
    ...
    const resume = await duplicateResumeForUser(id, user.id, payload || {});
    res.status(201).json({ resume });
  } catch (err) {
    res.status(err.status || 500).json({ detail: err.message || "Failed to duplicate resume" });
  }
});
